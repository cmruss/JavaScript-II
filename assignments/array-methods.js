...
runners.forEach(function(value) {
    fullNames.push(`${value.first_name} ${value.last_name}`)
})
console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.
let firstNames = runners.map(function(value) {
    return (value.first_name)
});
let firstNamesAllCaps = firstNames.map(function caps(item) {
...
let runnersLargeSizeShirt = runners.filter(function(value) {
...
let ticketPriceTotal = runners.reduce(function(acc, value) {
...
// Problem 1 For the size L shirts, we need a mass e-mail list to notify the runners of their shirts not being in.
const largeSizedEmails = runners.map((value) => {
    return (`${value.email}, ${value.first_name} ${value.last_name}`);
})
